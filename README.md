...
The latest serial Solution run executed **1070 test cases**: **1070 passed** and **0 failed**. The Variant arithmetic contract keeps the existing operator-specific behavior: `Currency * Double` remains Currency while `Currency + Double` remains Double. Empty is treated as an Integer operand for `/`, so `Empty / Single` preserves the Single result subtype. Managed `LSet` now also preserves VB6 Boolean fields through the 2-byte `VARIANT_BOOL` layout and native-width `LongPtr` fields through `IntPtr`. The Managed COM manifest CLI regression, the `SAFEARRAY(VT_UNKNOWN)` runtime regression, the MSBuild missing-input validation, single-project incremental/recovery tracking, native COM `EXCEPINFO` cleanup, culture-aware dispatch LCIDs, the typed `Clipboard.GetText` IR/runtime host contract, the `Command` process-argument contract, the extended `Err` HelpFile/HelpContext/LastDllError contract, and VB6 Collection error-number handling are present in the tree. No visible Win32 messagebox or dialog remained after verification.

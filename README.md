...
- the `Len`, two- and three-argument `Mid`, ASCII `Chr`, `InStr`, `InStrRev`, `Replace`, and current `Abs`/`Sgn`/`Fix`/`Round`/`Sqr`/`Exp`/`Log`/`Sin`/`Cos`/`Tan`/`Atn`/`Rnd`/`Randomize` math intrinsics, including `Null`-/`Empty`-semantics for `Abs`, `Fix` and `Round`, plus the `CByte`/`CInt`/`CLng`/`CLngPtr`/`CUShort`/`CUInt`/`CULng`/`CDec`/`CSng`/`CDbl`/`CBool`/`CStr` conversions and the `Left`/`Right`/`UCase`/`LCase`/`Trim`/`LTrim`/`RTrim`/`Asc`/`IsNumeric` string functions. Each intrinsic symbol carries the runtime method the backend calls, so it is resolved and checked like any other procedure and a user declaration of the same name still shadows it
...
- the regression suite currently contains **865 tests**, including typed string-key Variant, Object, named default-property and COM `DISPID_VALUE` dispatch
...
- an optional host boundary for compiled Forms/UserControls: `VB6.Runtime` exposes lifecycle, dynamic member, control-creation and enumeration hooks, while `VB6.Runtime.WinForms` maps standard VB6 controls, Twips, OLE colors, fonts and `Load`/`Unload`/`Show` to WinForms
...
The legacy project path is verified against `conformance/VISIA/4.8.7.1/prjVisia.vbp`: the CLI report analyzes all 40 project items without errors, and `--emit-assembly` produces a managed executable with its debug and runtime artifacts. The `.vbg` path is covered by dependency-order, library/executable-output, and project-group emission tests.

...
Current native LLVM status: checked integer Add/Subtract/Multiply and Currency Add/Subtract/Negate use trap-guarded i64 overflow helpers with explicit target-width guards. Currency multiplication, checked/rounded conversions, native `Err`/`On Error` recovery, and complex native ABI contracts remain open.

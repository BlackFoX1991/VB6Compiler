...
replacement arrays back through the native delegate boundary; UDT-, raw-pointer- and nested
string array ABIs remain separate interop work. Native-width `LongPtr()` arrays are supported for
explicit x86/x64 targets as `SAFEARRAY(VT_I4)`/`SAFEARRAY(VT_I8)`.

...
The latest serial Solution run executed **1043 test cases**: **1043 passed** and **0 failed**. The Variant arithmetic contract keeps the existing operator-specific behavior: `Currency * Double` remains Currency while `Currency + Double` remains Double. The Managed COM manifest CLI regression and the `SAFEARRAY(VT_UNKNOWN)` runtime regression are present in the tree. No visible Win32 messagebox or dialog remained after verification.

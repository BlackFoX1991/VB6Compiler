...
The suite currently holds **478 tests** across 157 test classes, and the Release build is warning-free. The current VISIA parity measurement is **1339 total errors** - **480 parser**, **62 lexer**, **797 semantic** - across 27 of 40 VISIA project items. Parser errors fell from 1214 to 480 with the `With` and member-access work, the largest single drop so far: `With` appears 629 times in 19 of those files, and without member access every following line derailed. `docs/ROADMAP.md` keeps the measured history and the current blocker ranking.

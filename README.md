...
- the `Len`/`LenB`, two- and three-argument `Mid`, ASCII `Chr`, `InStr`, `InStrRev`, `Replace`, and current `Abs`/`Sgn`/`Fix`/`Round`/`Sqr`/`Exp`/`Log`/`Sin`/`Cos`/`Tan`/`Atn`/`Rnd`/`Randomize` math intrinsics, including `Null`-/`Empty`-semantics for `Abs`, `Sgn`, `Fix`, `Round` and `Int`, plus the `CByte`/`CInt`/`CLng`/`CLngPtr`/`CUShort`/`CUInt`/`CULng`/`CDec`/`CSng`/`CDbl`/`CBool`/`CStr` conversions and the `Left`/`Right`/`UCase`/`LCase`/`Trim`/`LTrim`/`RTrim`/`Asc`/`IsNumeric` string functions. `LenB` preserves the Unicode byte count, Null propagation and generated UDT in-memory layout. Each intrinsic symbol carries the runtime method the backend calls, so it is resolved and checked like any other procedure and a user declaration of the same name still shadows it
...
- the regression suite currently contains **923 tests**, including typed string-key Variant, Object, compiled COM automation dispatch, conditional compilation, UDT `Len`/`LenB` layout, and legacy `.dsr` project emission

...
The compatibility matrix contains 187 expectations (173 implemented, 0 partial, 14 planned) with 168/187 documented-verified.
...
- `DefBool`/`DefByte`/`DefCur`/`DefDate`/`DefDbl`/`DefInt`/`DefLng`/`DefObj`/`DefSng`/`DefStr`/`DefVar`
  over letter ranges, comma lists and single letters, typing variables that were never declared;
  an identifier type suffix outranks the directive, overlapping ranges report `VB6S0070`, and an
  undeclared `String` starts as the empty string with `VarType` 8 rather than as an unset Variant
- VB6 evaluation order: `And` and `Or` evaluate both operands with no short-circuit, a `For`
  header evaluates its limit and step once before the first iteration, and a `With` evaluates its
...
- `GoSub`/`Return`, including a `Return` with nothing to return to: it ends the program with the
  runtime's own message. A `Return` in a procedure containing no `GoSub` previously produced
  invalid IL, so the CLR rejected the whole method and the program died before its first line
- `Global` and `Friend` declarations, bracketed reserved words such as `[Select]`, implicit `Enum`
  member values, and `ParamArray` with and without arguments
...
Measured on 2026-09-11 at `a946c2d` on `main` with uncommitted changes, run `20260911T065502Z-544e3133`:
...
| Standard serial run, 13 test projects | 1903 cases: 1903 passed, 0 failed, 0 skipped | Serial run across every test project |
| Native x86 run with `VB6_REQUIRE_NATIVE_OCX=1` | 93/93 passed, 0 skipped | Separate x86 run of the WinForms tests |
| Oracle cross-check against VB6 SP6 | 8/8 passed, 0 skipped | Compared against VB6 SP6; covers only the surface an oracle case asks about |
...
The matrix reports **187 expectations**: **173 implemented**, **0 partial**, **14 planned**;
**168 documented-verified**, **14 not-yet-verified**, **5 oracle-verified**.

...
- `LSet target = source` assignment syntax, including Managed execution for fixed-length String targets, same-type UDT copies, and cross-UDT raw-layout transfers with scalar, Boolean, and native-width `LongPtr` fields; dynamic strings, arrays, Variants, and other native ABI layouts remain guarded
...
- the regression suite currently contains **1195 test cases**, including complete Variant-array `ByRef` replacement, `LBound`/`UBound` type-mismatch handling, VBG project-reference closure, registered COM TypeLib fallback, distinct `Object()`/`Variant()` array descriptors through managed `ReDim`, Variant assignment, cloning, SAFEARRAY replacement and COM/Callback reflection adapters, typed string-key Variant, non-numeric string Variant comparison ordering, OLE-Date `DateTime` Variant conversions and arithmetic, consistent `Missing`/array error contracts for Variant math intrinsics, Object, compiled COM automation dispatch with TypeInfo-aware ByRef gating, CLR-/SAFEARRAY-backed Variant predicates and indexed access, typed SAFEARRAY Connection-Point adapters with VBArray bounds/write-back, DATE-/Variant()-/String()-SAFEARRAY AddressOf callbacks with replacement write-back, native-width `LongPtr()` SAFEARRAY callbacks and Declare arrays on x86/x64, `Object()`/Control() `Declare` SAFEARRAYs with native `VT_DISPATCH` object and `Nothing` write-back, `VT_UNKNOWN` object-array marshalling with `IUnknown*` and `Nothing` round-trip, native `VT_VARIANT` SAFEARRAY state preservation for `Empty`, `Null`, `Nothing`, `Missing`, `Error`, `Date` and `Currency`, external `Declare` SAFEARRAY return values with preserved bounds, COM `ObjPtr`/`IUnknown` identity, TypeLibrary-based COM event identity fallback, direct COM `ClassFactory`/`IDispatch` activation of an emitted library, registry-free COM manifest emission, conditional compilation, UDT `Len`/`LenB` layout and managed cross-UDT `LSet` including Boolean fields, deterministic Windows-1252 `Chr`/`Asc`, managed CommonDialog/TreeView/ImageList/RichTextBox/Shape/Line/Menu/PopupMenu/GraphicsLine/PaintPicture/UserControl adapters, native x86 standard-OCX activation and RichTextBox connection-point events, native TreeView `Nodes` dispatch and collection enumeration, `Format$` weekday/week/quarter tokens, Form lifecycle events, MDI container/child hosting, `.frx` resource extraction, a WinForms generated-assembly runner, legacy `.dsr` project emission, Empty/Single Variant division promotion, MSBuild SDK rejection of missing VB6 project inputs, single-project incremental/recovery builds, native COM `EXCEPINFO` BSTR cleanup, culture-aware dispatch LCIDs, the typed `Clipboard.GetText` IR/runtime host contract, the `Command` process-argument contract, the extended `Err` HelpFile/HelpContext/LastDllError contract, and VB6 Collection error-number handling, `Null & Null` Variant propagation, full-precision Decimal `Debug.Print`, compiled `Debug.Assert` elision, procedure-wide Portable-PDB scopes, and Currency-/Date-preserving `Abs`, `Fix` and `Int`
...
The canonical serial `build.ps1` run on 2026-08-27 supersedes the historical baseline below:
**1195 test cases**, **1195 passed**, **0 failed** across 13 test projects.
With `-RequireNativeOcx`, the same script additionally verifies the native x86 OCX path: **48/48 passed**.

The latest canonical serial run executed **1195 test cases**: **1195 passed** and **0 failed**. The Variant arithmetic contract keeps the existing operator-specific behavior: `Currency * Double` remains Currency while `Currency + Double` remains Double. Empty is treated as an Integer operand for `/`, so `Empty / Single` preserves the Single result subtype. Managed `LSet` now also preserves VB6 Boolean fields through the 2-byte `VARIANT_BOOL` layout and native-width `LongPtr` fields through `IntPtr`. `Object()` and `Variant()` array descriptors remain distinct through managed `ReDim`, Variant assignment, clone/resize, and SAFEARRAY replacement; the Reflection-based callback and COM-event adapters select the one-argument `FromObject` contract explicitly. Variant `&` keeps a single `Null` operand as an empty string and propagates `Null` for `Null & Null`. Decimal Variant values now retain their full precision in `Debug.Print`, and compiled `Debug.Assert` statements are elided without evaluating their conditions. Portable PDBs now retain a procedure-wide scope even when no user locals exist. Native `VT_VARIANT` SAFEARRAYs now preserve `Empty`, `Null`, `Nothing`, `Missing`, `Error`, `Date` and `Currency` elements through the managed Declare/COM roundtrip, and external Declare SAFEARRAY return values are converted back to bound-preserving `VBArray<T>` containers. The Managed COM manifest CLI regression, the `SAFEARRAY(VT_UNKNOWN)` runtime regression, VBG project-reference closure, registered COM TypeLib fallback, Variant-array bounds error handling, the MSBuild missing-input validation, single-project incremental/recovery tracking, native COM `EXCEPINFO` cleanup, culture-aware dispatch LCIDs, the typed `Clipboard.GetText` IR/runtime host contract, the `Command` process-argument contract, the extended `Err` HelpFile/HelpContext/LastDllError contract, VB6 Collection error-number handling, and Currency-/Date-preserving scalar math are present in the tree. No visible Win32 messagebox or dialog remained after verification.

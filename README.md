...
The latest serial Solution run executed **1046 test cases**: **1046 passed** and **0 failed**. The Variant arithmetic contract keeps the existing operator-specific behavior: `Currency * Double` remains Currency while `Currency + Double` remains Double. Empty is treated as an Integer operand for `/`, so `Empty / Single` preserves the Single result subtype. Managed `LSet` now also preserves VB6 Boolean fields through the 2-byte `VARIANT_BOOL` layout. The Managed COM manifest CLI regression and the `SAFEARRAY(VT_UNKNOWN)` runtime regression are present in the tree. No visible Win32 messagebox or dialog remained after verification.

...
public sealed class ManagedEmitter
{
    ...
    private sealed class EmitContext
    {
        ...
        /// </summary>
        private void EmitAddressableCellWriteBack(
            InstructionEncoder encoder,
            IrProcedure procedure,
            IrPlace place)
        {
            ...
            switch (root)
            {
                case IrLocalPlace local when TryGetAddressableCell(procedure, local.Local, out var cell):
                    encoder.LoadLocal(cell.Id);
                    encoder.LoadLocal(local.Local.Id);
                    EmitAddressableCellBox(encoder, local.Type);
                    encoder.Call(GetRuntimeMethodReference(
                        AddressableStorageMethod(local.Type, "Write")));
                    break;
                case IrGlobalPlace global when TryGetGlobalCell(global.Global, out var globalCell):
                    ...
                    encoder.Call(GetRuntimeMethodReference(
                        AddressableStorageMethod(global.Type, "WriteIfPresent")));
                    break;
        ...
        private void EmitAddressableCellInitializers(InstructionEncoder encoder, IrProcedure procedure)
        {
            ...
            {
                if (pair.Key.Type is UserDefinedTypeSymbol)
                {
                    // Der CLR-Speicherplatz ist beim Eintritt genullt, und genau so faengt ein
                    // VB6-Datensatz an. Jede spaetere Memberzuweisung schreibt in die Zelle nach.
                    encoder.LoadLocal(pair.Key.Id);
                }
                else if (pair.Key.Type == TypeSymbol.String)
                {
                ...
                else if (pair.Key.Type == TypeSymbol.LongLong || pair.Key.Type == TypeSymbol.ULong)
                {
                    encoder.LoadConstantI8(0L);
                }
                ...
                EmitAddressableCellBox(encoder, pair.Key.Type);
            ...
            switch (place)
            {
                case IrLocalPlace local:
                    if (TryGetAddressableCell(procedure, local.Local, out var cell))
                    {
                        ...
                        EmitAddressableCellCast(encoder, local.Type);
                        break;
                    }
                    encoder.LoadLocal(local.Local.Id);
                    break;
                case IrParameterPlace parameter:
                    if (TryGetAddressableCell(procedure, parameter.Parameter, out var parameterLoadCell))
                    {
                        ...
                        EmitAddressableCellCast(encoder, parameter.Type);
                        break;
                    }
                    encoder.LoadArgument(GetIlArgumentIndex(procedure, parameter.Parameter.Index));
                ...
                case IrGlobalPlace global:
                    if (TryGetGlobalCell(global.Global, out var globalLoadCell))
                    {
                        ...
                        EmitAddressableCellCast(encoder, global.Type);
                        break;
                    }
                    encoder.OpCode(ILOpCode.Ldsfld);
                    encoder.Token(_globalHandles[global.Global]);
        ...
        private void EmitStore(InstructionEncoder encoder, IrProcedure procedure, IrPlace place, IrExpression value)
        {
            switch (place)
            {
                ...
                case IrFieldPlace field:
                    ...
                    encoder.Token(_fieldHandles[field.Field]);
                    // Ein Member schreibt in den CLR-Speicherplatz des Datensatzes; die Zelle
                    // waere danach veraltet, wenn sie nicht mitgeschrieben wird.
                    EmitAddressableCellWriteBack(encoder, procedure, field);
                    break;
                case IrArrayElementPlace element:
            ...
            IrAddressableGlobalPointerExpression pointer)
        {
            if (TryGetGlobalCell(pointer.Global, out var cell))
            {
                ...
                EmitAddressableCellBox(encoder, pointer.Global.Type);
        ...
        /// </summary>
        private void EmitAddressableCellCast(InstructionEncoder encoder, TypeSymbol type)
        {
            if (type is not UserDefinedTypeSymbol)
            {
                return;
            }

            encoder.OpCode(ILOpCode.Unbox_any);
            encoder.Token(GetTypeEntityHandle(type));
        }

        private void EmitAddressableCellBox(InstructionEncoder encoder, TypeSymbol type)
        {
            if (type is not UserDefinedTypeSymbol)
            {
                return;
        ...
        /// <summary>
        /// Pushes the native address a stored pointer answers with: the cell itself, or a member
        /// inside it. The cell is on the stack already.
        /// </summary>
            ...
            string? memberPath)
        {
            ...
            {
                encoder.LoadString(_metadata.GetOrAddUserString(memberPath));
                encoder.Call(GetRuntimeMethodReference(
                    AddressableStorageMethod(type, "MemberNativeAddress")));
            }
        ...
        private static MethodInfo AddressableStorageMethod(TypeSymbol type, string operation)
        {
            ...
            return operation switch
            {
                ...
                "MemberNativeAddress" => Static(
                    typeof(VBAddressableStorage),
                    nameof(VBAddressableStorage.GetRecordMemberNativeAddress),
                    typeof(object),
                    typeof(string)),
        ...
        private void EmitProcedureCall(InstructionEncoder encoder, IrProcedure procedure, IrProcedureCallExpression call)
        {
            ...
            foreach (var argument in call.Arguments)
            {
                    ...
                    argument.Expression is IrAddressExpression address)
                {
                    ...
                    EmitAddressableCellWriteBack(encoder, procedure, address.Place);
                }
        ...
        private MethodInfo ResolveRuntimeMethod(IrRuntimeCallExpression call, out int skippedArgument)
        {
            ...
            if (m == IrRuntimeMethod.GraphicsPSet) return Static(
                ...
                typeof(object),
                typeof(bool));
            if (m == IrRuntimeMethod.GraphicsPSetOnTarget) return Static(
                typeof(VBInteraction),
                nameof(VBInteraction.GraphicsPSet),
                typeof(object),
                typeof(float),
            ...
            if (m == IrRuntimeMethod.ConvertCInt) return Static(typeof(VBConversions), "ConvertCInt", typeof(object));
            if (m == IrRuntimeMethod.ConvertCLng) return Static(typeof(VBConversions), "ConvertCLng", typeof(object));
            if (m == IrRuntimeMethod.ConvertCLngPtr) return Static(typeof(VBConversions), "ConvertCLngPtr", typeof(object));
            ...
            if (m == IrRuntimeMethod.ObjectLifetimeRunTerminator) return Static(typeof(VBObjectLifetime), nameof(VBObjectLifetime.RunTerminator), typeof(object));
            ...
            if (m == IrRuntimeMethod.InteractionLoadControlArrayElement)
                return Static(
                    typeof(VBInteraction),
                    nameof(VBInteraction.LoadControlArrayElement),
                    typeof(VBArray<object?>),
                    typeof(int),
                    typeof(string),
                    typeof(object));
            if (m == IrRuntimeMethod.InteractionUnloadControlArrayElement)
                return Static(
                    typeof(VBInteraction),
                    nameof(VBInteraction.UnloadControlArrayElement),
                    typeof(VBArray<object?>),
                    typeof(int),
                    typeof(string),
                    typeof(object));
            ...
            if (m == IrRuntimeMethod.InteractionSetMember) return Static(typeof(VBInteraction), nameof(VBInteraction.SetMember), typeof(object), typeof(string), typeof(object));
            ...
            if (m == IrRuntimeMethod.InteractionPrinter) return Static(typeof(VBInteraction), nameof(VBInteraction.Printer));
            if (m == IrRuntimeMethod.InteractionPrinterGetString) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterGetString), typeof(string));
            if (m == IrRuntimeMethod.InteractionPrinterSetString) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterSetString), typeof(string), typeof(string));
            if (m == IrRuntimeMethod.InteractionPrinterGetLong) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterGetLong), typeof(string));
            if (m == IrRuntimeMethod.InteractionPrinterSetLong) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterSetLong), typeof(string), typeof(int));
            if (m == IrRuntimeMethod.InteractionPrinterGetSingle) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterGetSingle), typeof(string));
            if (m == IrRuntimeMethod.InteractionPrinterSetSingle) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterSetSingle), typeof(string), typeof(float));
            if (m == IrRuntimeMethod.InteractionPrinterGetBoolean) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterGetBoolean), typeof(string));
            if (m == IrRuntimeMethod.InteractionPrinterSetBoolean) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterSetBoolean), typeof(string), typeof(bool));
            if (m == IrRuntimeMethod.InteractionPrinterGetObject) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterGetObject), typeof(string));
            if (m == IrRuntimeMethod.InteractionPrinterSetObject) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterSetObject), typeof(string), typeof(object));
            if (m == IrRuntimeMethod.InteractionPrinterPrint) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterPrint), typeof(object));
            if (m == IrRuntimeMethod.InteractionPrinterNewPage) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterNewPage));
            if (m == IrRuntimeMethod.InteractionPrinterEndDoc) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterEndDoc));
            if (m == IrRuntimeMethod.InteractionPrinterKillDoc) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterKillDoc));
            if (m == IrRuntimeMethod.InteractionPrinterTextWidth) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterTextWidth), typeof(string));
            if (m == IrRuntimeMethod.InteractionPrinterTextHeight) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterTextHeight), typeof(string));
            if (m == IrRuntimeMethod.InteractionPrinterScaleX) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterScaleX), typeof(float), typeof(int), typeof(int));
            if (m == IrRuntimeMethod.InteractionPrinterScaleY) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterScaleY), typeof(float), typeof(int), typeof(int));
            if (m == IrRuntimeMethod.InteractionPrinterPaintPicture) return Static(typeof(VBInteraction), nameof(VBInteraction.PrinterPaintPicture), typeof(object), typeof(float), typeof(float), typeof(float), typeof(float));

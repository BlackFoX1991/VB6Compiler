...
public static class PortablePdbEmitter
{
    ...
    public static byte[] Emit(IrProgram program, byte[] peImage, ManagedEmitOptions options)
    {
        ...
        foreach (var procedure in procedures)
        {
            pdbMetadata.AddMethodDebugInformation(
                procedureDocuments.TryGetValue(procedure, out var document) ? document : default,
                default);
        }

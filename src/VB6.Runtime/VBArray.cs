...
public sealed class VBDeclareArrayBuffer : IDisposable
{
    ...
    public VBArray<T>? GetManagedArray<T>()
    {
        ...
        try
        {
            ...
            var value = (_expectedType & 0x0FFF) == (ushort)VarEnum.VT_VARIANT &&
                        VBComDispatch.TryReadVariantArrayFromNativeVariant(_variant, out var variantArray)
                ? variantArray
                : Marshal.GetObjectForNativeVariant(_variant);

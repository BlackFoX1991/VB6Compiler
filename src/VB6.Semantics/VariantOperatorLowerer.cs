...
/// runtime. Multiplication restores Variant operands/result semantics; equality restores the
/// original comparison operands while retaining a Boolean result. Other Variant operators remain
    ...
    private static BoundExpression LowerExpression(BoundExpression expression)
    {
        switch (expression)
        {
            ...
            case BoundBinaryExpression binary:
            {
                ...
                var hasVariantOperand = OriginatesFromVariant(left) || OriginatesFromVariant(right);

                if (binary.OperatorKind == SyntaxKind.StarToken && hasVariantOperand)
                {
                ...
                if (binary.OperatorKind == SyntaxKind.EqualsToken && hasVariantOperand)
                {
                    return new BoundBinaryExpression(
                        RestoreVariantComparisonOperand(left),
                        binary.OperatorKind,
                        RestoreVariantComparisonOperand(right),
                        TypeSymbol.Boolean);
                }

...
Messung vom 2026-09-06 auf `main` / `230d1f1`, Lauf `20260906T163223Z-9a43fd40`:
...
| Standardlauf, 13 Testprojekte | 1755 Fälle: 1755 bestanden, 0 fehlgeschlagen | Serieller Lauf über alle Testprojekte |
...
Die vorhandenen Ausführungstests decken für erzeugte Klassen inzwischen Aliase, Selbstzuweisung, ByRef/ByVal, Rückgaben, Felder, Variant-/Array-/Collection-Speicher, `WithEvents`, behandelte Fehler, Initialisierungsfehler, reentrante Terminierung, Zyklen, `End` und referenzierte Projektassemblies ab. Runtime-Tests gegen `Scripting.Dictionary` bestätigen zusätzlich, dass eine direkte Aktivierung über Alias-, Array- und Collection-Slots lebt und nach dem letzten Slot ungültig wird. Ein Emitter-Test trennt diese direkte Aktivierung von einer Funktionsrückgabe, die ihren vorhandenen Speicherbesitz nur weiterreicht. Das schließt nicht den gesamten COM-Vertrag: von fremden Membern zurückgegebene RCWs, externe Hosts mit eigenen Wrapper-Anteilen und die genaue native Referenzzählung brauchen unabhängige Fremdclient-Probes. Daher bleibt R2 offen.

...
Messung vom 2026-09-06 auf `main` / `e52459f` mit nicht committeten Änderungen, Lauf `20260906T095035Z-33c8a337`:
...
| Standardlauf, 13 Testprojekte | 1737 Fälle: 1737 bestanden, 0 fehlgeschlagen | Serieller Lauf über alle Testprojekte |
...
lokalen `Long`, `Integer`, `Byte`, `Boolean`, `Single`, `Double` oder `Date` speichern. Von der
Runtime besessene native Vier-, Zwei-, Ein- und Acht-Byte-Zellen bleiben über GC stabil, werden an
normalen Loads/Stores und CLR-ByRef-Write-backs synchronisiert und bei der Prozedurrückkehr
freigegeben; `Boolean` nutzt dabei die klassische zwei Byte breite `-1`/`0`-Darstellung, `Single`
und `Double` ihre vier beziehungsweise acht Byte breiten IEEE-754-Layouts und `Date` das acht Byte
breite OLE-Automation-Layout. AnyCPU/x64 sowie alle anderen Speicherfamilien behalten ihre

...
`VarPtr(localLong)` und `VarPtr(localInteger)` native Vier- beziehungsweise Zwei-Byte-Zellen.
...
nur lokale `Long`- und `Integer`-Slots im x86-Managed-Pfad. Außerhalb dieser Fälle und außerhalb

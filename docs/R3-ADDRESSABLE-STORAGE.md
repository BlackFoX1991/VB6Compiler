...
`VarPtr(localSingle)`, `VarPtr(localDouble)` und `VarPtr(localDate)` native Vier-, Zwei-, Ein-,
zwei-, vier-, acht- und acht Byte breite Zellen. Der Boolean-Zellwert verwendet dabei VB6s
`-1`/`0`-Darstellung statt eines CLR-`bool`; Single und Double nutzen ihre unveränderten
IEEE-754-Layouts und Date das acht Byte breite OLE-Automation-Datum. Normale Loads/Stores sowie
...
nur lokale `Long`-, `Integer`-, `Byte`-, `Boolean`-, `Single`-, `Double`- und `Date`-Slots im

...
public sealed class ComDispatchRuntimeTests
{
    ...
    [TestMethod]
    [SupportedOSPlatform("windows")]
    public void DeclareArrayBuffer_MarshalsUnknownSafeArrayObjectsAndNothing()
    {
        if (!OperatingSystem.IsWindows())
        {
            Assert.Inconclusive("The IUnknown SAFEARRAY test requires Windows.");
            return;
        }
        ...
        try
        {
            ...
            using var buffer = VBDeclareArrayBuffer.Create(
                source,
                (ushort)((ushort)VarEnum.VT_ARRAY | (ushort)VarEnum.VT_UNKNOWN));

            var safeArray = Marshal.ReadIntPtr(buffer.GetNativeAddress());
            Assert.AreNotEqual(IntPtr.Zero, safeArray);
            Assert.AreEqual(0, SafeArrayGetVartype(safeArray, out var actualType));
            Assert.AreEqual((ushort)VarEnum.VT_UNKNOWN, actualType);

            var result = buffer.GetManagedArray<object>();
            Assert.IsNotNull(result);
            Assert.IsTrue(Marshal.IsComObject(result![-1]));
            Assert.IsTrue(VBVariants.IsNothing(result[0]));
        }
        finally
        {
            if (Marshal.IsComObject(dictionary))

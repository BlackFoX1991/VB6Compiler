...
public sealed class LlvmEmitterTests
{
    ...
    public void Emit_LowersNativeErrorBoundariesAndStatus()
    {
        ...
        StringAssert.Contains(result.ModuleText, "@__vb6_error_resume_boundary_state = thread_local global i32 -1");
        ...
        StringAssert.Contains(result.ModuleText, "__vb6_error_try_0:");
        StringAssert.Contains(result.ModuleText, "br label %bb1");
        StringAssert.Contains(result.ModuleText, "call i32 @__vb6_error_number()");
        StringAssert.Contains(result.ModuleText, "call i32 @__vb6_error_resume_boundary()");
        StringAssert.Contains(result.ModuleText, "call void @__vb6_error_clear()");
        StringAssert.Contains(result.ModuleText, "i32 0, label %__vb6_error_continue_0");
    }

    [TestMethod]
    public void Emit_LowersNativeResumeToRetryBoundary()
    {
        var divide = new IrRuntimeCallExpression(
            IrRuntimeMethod.IntegerDivideLong,
            ImmutableArray.Create<IrCallArgument>(
        ...
        var procedure = new IrProcedure(
            ...
            ImmutableArray.Create(
                ...
                new IrBasicBlock(
                    ...
                    "handler",
                    ImmutableArray.Create<IrInstruction>(
                        new IrResumeInstruction(IrResumeKind.Same)),
                    new IrReturnTerminator(new IrConstantExpression(42L, TypeSymbol.Long)))));

        var result = new LlvmEmitter().Emit(CreateProgram(procedure), new LlvmEmitOptions(LlvmArchitecture.X64));

        Assert.IsTrue(result.Success, string.Join(Environment.NewLine, result.Diagnostics));
        StringAssert.Contains(result.ModuleText, "switch i32");
        StringAssert.Contains(result.ModuleText, "i32 0, label %__vb6_error_try_0");
    }
